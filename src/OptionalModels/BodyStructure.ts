...
export class BodyStructure extends fhirModels.DomainResource implements fhirInterfaces.IBodyStructure {
  ...
  constructor(source:Partial<fhirInterfaces.IBodyStructure>) {
    ...
    if (source["location"] !== undefined) { this.location = new fhirModels.CodeableConcept(source.location); }
    if (source["locationQualifier"] !== undefined) { this.locationQualifier = source.locationQualifier.map((x) => new fhirModels.CodeableConcept(x)); }
    if (source["morphology"] !== undefined) { this.morphology = new fhirModels.CodeableConcept(source.morphology); }
