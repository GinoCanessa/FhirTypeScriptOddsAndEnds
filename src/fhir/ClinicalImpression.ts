 ...
 * One or more sets of investigations (signs, symptoms, etc.). The actual grouping of investigations varies greatly depending on the type and context of the assessment. These investigations may include data generated during the assessment process, or data previously generated and recorded that is pertinent to the outcomes.
 */
export class ClinicalImpressionInvestigation extends fhir.BackboneElement implements fhir.IClinicalImpressionInvestigation {
  /**
   * A name/code for the group ("set") of investigations. Typically, this will be something like "signs", "symptoms", "clinical", "diagnostic", but the list is not constrained, and others such groups such as (exposure|family|travel|nutritional) history may be used.
   */
  ...
  constructor(source:Partial<fhir.IClinicalImpressionInvestigation> = {}) {
    ...
    if (source["item"]) { this.item = source.item.map((x:Partial<fhir.IReference>) => new fhir.Reference(x)); }
  }
  /**
 ...
 * Specific findings or diagnoses that were considered likely or relevant to ongoing treatment.
 */
export class ClinicalImpressionFinding extends fhir.BackboneElement implements fhir.IClinicalImpressionFinding {
  /**
   * Which investigations support finding or diagnosis.
   */
 ...
 * A record of a clinical assessment performed to determine what problem(s) may affect the patient and before planning the treatments or management strategies that are best to manage a patient's condition. Assessments are often 1:1 with a clinical consultation / encounter,  but this varies greatly depending on the clinical workflow. This resource is called "ClinicalImpression" rather than "ClinicalAssessment" to avoid confusion with the recording of assessment tools such as Apgar score.
 */
export class ClinicalImpression extends fhir.DomainResource implements fhir.IClinicalImpression {
  /**
   * Resource Type Name
   */
   ...
   * This element is labeled as a modifier because the status contains the code entered-in-error that marks the clinical impression as not currently valid.
   */
  public status: ClinicalImpressionStatusEnum|null;
  ...
  constructor(source:Partial<fhir.IClinicalImpression> = {}) {
    ...
    if (source["finding"]) { this.finding = source.finding.map((x:Partial<fhir.IClinicalImpressionFinding>) => new fhir.ClinicalImpressionFinding(x)); }
    if (source["identifier"]) { this.identifier = source.identifier.map((x:Partial<fhir.IIdentifier>) => new fhir.Identifier(x)); }
    if (source["investigation"]) { this.investigation = source.investigation.map((x:Partial<fhir.IClinicalImpressionInvestigation>) => new fhir.ClinicalImpressionInvestigation(x)); }
    ...
    if (source["problem"]) { this.problem = source.problem.map((x:Partial<fhir.IReference>) => new fhir.Reference(x)); }
    if (source["prognosisCodeableConcept"]) { this.prognosisCodeableConcept = source.prognosisCodeableConcept.map((x:Partial<fhir.ICodeableConcept>) => new fhir.CodeableConcept(x)); }
    if (source["prognosisReference"]) { this.prognosisReference = source.prognosisReference.map((x:Partial<fhir.IReference>) => new fhir.Reference(x)); }
    if (source["protocol"]) { this.protocol = source.protocol.map((x) => (x)); }
    if (source["_protocol"]) { this._protocol = source._protocol.map((x:Partial<fhir.IFhirElement>) => new fhir.FhirElement(x)); }
    this.status = null;
