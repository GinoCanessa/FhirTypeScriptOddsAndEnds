...
export class SearchParameterComponent extends fhir.BackboneElement {
  readonly __dataType:string = 'SearchParameterComponent';
  /**
   * The definition of the search parameter that describes this part.
   */
...
export class SearchParameter extends fhir.DomainResource {
  readonly __dataType:string = 'SearchParameter';
  /**
   * Resource Type Name
   */
   ...
   * Types of resource (if a resource is referenced).
   */
  public target?: fhir.FhirCode[]|undefined = [];
  /**
   * Whether multiple values are allowed for each time the parameter exists. Values are separated by commas, and the parameter matches if any of the values match.
   */
   ...
   * Systems are not required to list all the chain names they support, but if they don't list them, clients might not know to use them.
   */
  public chain?: fhir.FhirString[]|undefined = [];
  /**
   * Used to define the parts of a composite search parameter.
   */
  ...
  public override doModelValidation():fhir.OperationOutcome {
    ...
    if (this["derivedFrom"]) { outcome.issue!.push(...this.derivedFrom.doModelValidation().issue!); }
    if (!this['status']) {
    ...
    if (this["base"]) { this.base.forEach((x) => { outcome.issue!.push(...x.doModelValidation().issue!); }) }
    if (!this['type']) {
    ...
    if (this["multipleOr"]) { outcome.issue!.push(...this.multipleOr.doModelValidation().issue!); }
    if (this["multipleAnd"]) { outcome.issue!.push(...this.multipleAnd.doModelValidation().issue!); }
    if (this["chain"]) { this.chain.forEach((x) => { outcome.issue!.push(...x.doModelValidation().issue!); }) }
