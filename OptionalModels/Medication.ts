...
export class Medication extends fhirModels.DomainResource implements fhirInterfaces.IMedication {
  ...
  constructor(source:Partial<fhirInterfaces.IMedication>) {
    ...
    if (source["amount"] !== undefined) { this.amount = new fhirModels.Ratio(source.amount); }
    if (source["batch"] !== undefined) { this.batch = new fhirModels.MedicationBatch(source.batch); }
    ...
    if (source["ingredient"] !== undefined) { this.ingredient = source.ingredient.map((x) => new fhirModels.MedicationIngredient(x)); }
